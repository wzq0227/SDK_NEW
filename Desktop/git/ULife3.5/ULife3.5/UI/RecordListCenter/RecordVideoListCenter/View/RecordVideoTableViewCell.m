...
@interface RecordVideoTableViewCell ()
 ...
 *  编辑模式imgView
 */
@property (strong, nonatomic) EnlargeClickButton *selectVideoBtn;

@end


@implementation RecordVideoTableViewCell
...
- (void)setupUI{
    ...
    [self.contentView addSubview:self.selectVideoBtn];
...
- (void)updateEditStyleConstraints{
    ...
    [self.selectVideoBtn mas_remakeConstraints:^(MASConstraintMaker *make) {
...
- (void)selectClick:(UIButton *)selectBtn{
    self.selectVideoBtn.selected = !self.selectVideoBtn.selected;

    if (self.selectVideoBtn.selected) {
        //选中
        ...
        if (self.recordVideoTableViewCellData) {
            self.recordVideoTableViewCellData.isSelect = YES;
        }
        ...
        if (self.recordVideoTableViewCellData) {
            self.recordVideoTableViewCellData.isSelect = NO;
        }
        if (self.myStateBlock) {
...
- (void)setSelectBtnEnabled:(BOOL)enabled{
    self.selectVideoBtn.userInteractionEnabled = enabled;
}

#pragma mark -- 数据源处理
- (void)setRecordVideoTableViewCellData:(RecordVideoTableViewCellModel *)recordVideoTableViewCellData
{
    if (!recordVideoTableViewCellData)
    {
        return ;
    }
    _recordVideoTableViewCellData = recordVideoTableViewCellData;
    self.dateLabel.text           = recordVideoTableViewCellData.recordVideoDateStr;
    self.timeLabel.text           = recordVideoTableViewCellData.recordVideoTimeStr;
    self.fileNameLabel.text       = recordVideoTableViewCellData.recordVideoFileNameStr;
    self.fileSizeLabel.text       = recordVideoTableViewCellData.recordVideoFileSizeStr;
    self.typeImageView.image      = [UIImage imageNamed:@"Record_VideoIcon@2x.png"];
    recordVideoTableViewCellData.isDownload = [mFileManager fileExistsAtPath:recordVideoTableViewCellData.recordFilePath];
    [self setDownloadStatusImage:recordVideoTableViewCellData.isDownload];
    [self setSelectBtnStatus:recordVideoTableViewCellData.isSelect];
...
- (void)setLocalVideoModel:(LocalVideoModel *)localVideoModel{
    ...
    self.timeLabel.text           = localVideoModel.recordVideoTimeStr;
...
- (void)setMediaFileCellData:(MediaFileModel *)mediaFileCellData
{
    ...
    self.timeLabel.text      = mediaFileCellData.createTime;
...
- (void)setSelectBtnStatus:(BOOL)isSelect{
    if (isSelect) {
        //选中
        [self.selectVideoBtn setBackgroundImage:[UIImage imageNamed:@"deleteBtnHeighLight"] forState:UIControlStateNormal];
        self.selectVideoBtn.selected = YES;
//        self.myStateBlock(YES);
    }
    else{
        //未选中
        [self.selectVideoBtn setBackgroundImage:[UIImage imageNamed:@"deleteBtnNormal"] forState:UIControlStateNormal];
        self.selectVideoBtn.selected = NO;
//        self.myStateBlock(NO);
    }
}
 ...
 *  编辑模式选择 imageBtn
 */
- (EnlargeClickButton *)selectVideoBtn{
    if (!_selectVideoBtn) {
        _selectVideoBtn = [EnlargeClickButton buttonWithType:UIButtonTypeCustom];
        [_selectVideoBtn setBackgroundImage:[UIImage imageNamed:@"Record_FileUnselected"] forState:UIControlStateNormal];
        _selectVideoBtn.hidden = YES;
        [_selectVideoBtn addTarget:self action:@selector(selectClick:) forControlEvents:UIControlEventTouchUpInside];
    }
