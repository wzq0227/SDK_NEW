...
int goscam_text_compress(const char* src, unsigned int src_len, char** dst, quicklz_log_callback cb) {
  if(*dst !=NULL)
    return -1;
  ...
  compressed = (char*) malloc(src_len + 400);
  ret = qlz_compress(src, compressed, src_len, state_compress);
  if(ret < 0) {
    if(cb != NULL)
	    cb("qlz_compress failed, ret=%d", ret);
    goto goscam_text_compress_error;
  }
  *dst = (char*) malloc(ret);
  memcpy(*dst, compressed, ret);
goscam_text_compress_error:
  free(compressed);
  free(state_compress);
  compressed = NULL;
  state_compress = NULL;
...
int goscam_text_uncompress(const char* src, unsigned int src_len, char** dst, quicklz_log_callback cb) {
  ...
  ret = qlz_size_decompressed(src);
  // ret = qlz_size_compressed(src);
  if(ret < 0) {
    if(cb != NULL)
	    cb("qlz_size_decompressed failed, ret=%d", ret);
    goto goscam_text_uncompress_error;
  }
  *dst = (char*) malloc(ret);
  // printf("malloc: ret=%d\n", ret);
  // decompress and write result
  ret = qlz_decompress(src, *dst, state_decompress);
  if(strlen(*dst) > ret) {
    *((*dst)+ret) = '\0';
  } else if(cb != NULL) {
	    cb("goscam_text_uncompress strlen(*dst)[=%d] > ret[=%d]", strlen(*dst), ret);
  }
  // printf("malloc: ret=%d, size=%d\n", ret, strlen(*dst));
goscam_text_uncompress_error:
