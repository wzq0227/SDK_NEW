...
-(int)rtk_sc_config_start: (NSString *)ssid psw:(NSString *)password pin:(NSString *)pin
{
    ...
    // set profile
    NSLog(@"rtk_pattern_build_profile,simpleconfig: set pattern %d", m_current_pattern+1);
                ...
                if (pattern_mode==MODE_CONFIG)
                {
                    ...
                    m_config_duration++;
                    {
                        ...
                        if (m_current_pattern==PATTERN_FOUR)
                        {
                            ...
                            NSLog(@"rtk_pattern_send,m_current_pattern = %d,status = %d",m_current_pattern,status);
                        }
                        else
                        {
                            status = [m_pattern[m_current_pattern] rtk_pattern_send:[NSNumber numberWithInt:SC_SEND_ROUND_PER_SEC]];
                            NSLog(@"rtk_pattern_send,m_current_pattern != %d",m_current_pattern);
                        }
                    }
#endif
