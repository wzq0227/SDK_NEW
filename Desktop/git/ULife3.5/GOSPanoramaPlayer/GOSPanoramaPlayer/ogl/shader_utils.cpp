...
GLuint loadShader(GLenum shaderType, const char* pSource) {
    ...
    if (shader) {
       	printf("zzz: %s %d %s\n",__FUNCTION__,__LINE__,pSource);
        glShaderSource(shader, 1, &pSource, NULL);
       	printf("zzz: %s %d %d\n",__FUNCTION__,__LINE__,shader);
        glCompileShader(shader);
        GLint compiled = 0;
        glGetShaderiv(shader, GL_COMPILE_STATUS, &compiled);
       	printf("zzz: %s %d %d\n",__FUNCTION__,__LINE__,compiled);
        if (!compiled) {
            GLint infoLen = 0;
...
GLuint createProgram(const char* pVertexSource, const char* pFragmentSource) {
    ...
    printf("zzz: %s %d %d\n",__FUNCTION__,__LINE__, vertexShader);
    if (!vertexShader) {
    ...
    printf("zzz: %s %d %d\n",__FUNCTION__,__LINE__, pixelShader);
    if (!pixelShader) {
        return 0;
    }
        ...
        if (linkStatus != GL_TRUE) {
            ...
            if (bufLength) {
                ...
                if (buf) {
                    ...
                    printf("glGetProgramInfoLog: %s %d %d\n",__FUNCTION__,__LINE__, buf);
                    free(buf);
                }
            }
            glDeleteProgram(program);
