...
int write_audio_file(audio_file *aufile, void *sample_buffer, int samples, int offset, unsigned char *pOutBuf)
{
    char *buf = (char *)sample_buffer;
    switch (aufile->outputFormat)
    {
    case FAAD_FMT_16BIT:
        return write_audio_16bit(aufile, buf + offset*2, samples, pOutBuf);
    case FAAD_FMT_24BIT:
...
static int write_audio_16bit(audio_file *aufile, void *sample_buffer,
                             unsigned int samples, unsigned char *pOutBuf)
{
